...
function updateHeroContent(user) {
  ...
  const startButton = document.querySelector('.hero-content .shop-now');
	...
	if (user) {
		// User is signed in
		heroText.textContent = " Manage your bookings, view tailored guest itineraries, and access crew tools in one place.";
		startButton.textContent = "Explore";
		// Ensure the start button navigates to the digital (member) area
		if (startButton) {
			startButton.setAttribute('role', 'link');
			startButton.setAttribute('aria-label', 'Explore member area');
		...
		// User is signed out
		heroText.textContent = "Welcome to Winter Suites Hotel — a luxury case study in hospitality excellence. Discover bespoke experiences, seasonal winter packages, and our commitment to refined comfort and impeccable service.";
		startButton.textContent = "Explore Winter Suites";
		// When signed out, send users to the login page
		if (startButton) {
			startButton.setAttribute('role', 'link');
			startButton.setAttribute('aria-label', 'Explore — sign in');
			startButton.onclick = function (e) {
				e.preventDefault();
